...
product_predicted_data_output_file = "output-supadhy2-B00627613.txt"
...
while(i<=100){
  ...
  product_dailysale_predict_matrix[i,]=matrix(c(product_matrix[1,i],as.numeric(round(forcast_mean))),1,119)
  i <- i+1
  #plotting the forcasted data of i product on graph
  plot(forcast_arima_data_rest)
}
